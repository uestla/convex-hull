...
module ConvexHull
{
	...
	export class Solver
	{
		...
		static quickHull(points: Point[]): Result
		{
			...
			Solver._quickHull(min, max, points, result);
			Solver._quickHull(max, min, points, result);
		...
		private static _quickHull(a: Point, b: Point, points: Point[], result: Result): void
		{
			...
			Solver._quickHull(a, farthest, halfs[0], result);
			Solver._quickHull(farthest, b, halfs[1], result);
		}
